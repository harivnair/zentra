...
export default function EnquiriesPage() {
    ...
    const openEditModal = (enquiry: Enquiry) => {
        ...
        const editData: EnquiryFormData = {
            ...
            eventType:
                (enquiry.eventType as "PERSONAL" | "CORPORATE" | "OTHER" | undefined) ||
                "CORPORATE",
    ...
    const handleDownload = (id: string) => {
        ...
        if (enquiry) {
            ...
            const csvContent = `Client,Date,POC,Status,Assignee\n${
                clientMap.get(enquiry.client) || enquiry.client
            },${enquiry.date},${enquiry.poc},${enquiry.status},${enquiry.assignee}`;
            const blob = new Blob([csvContent], { type: "text/csv" });
            const url = window.URL.createObjectURL(blob);
            const a = document.createElement("a");
            a.href = url;
            a.download = `enquiry-${id}-${(clientMap.get(enquiry.client) || enquiry.client).replace(
                /[^a-zA-Z0-9]/g,
                "_"
            )}.csv`;
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-muted-foreground">
                            You have {enquiriesCount}{" "}
                            {enquiriesCount === 1 ? "Enquiry" : "Enquiries"}
                        </p>
                    </div>
                </div>
                                ...
                                >
                                    <div className="flex items-center justify-between">
                                        <div className="font-medium">
                                            {clientMap.get(e.client) || e.client}
                                        </div>
                                        <div
                                    ...
                                    <div className="mt-3 text-sm">
                                        Assignee: <span className="font-medium">{e.assignee}</span>
                                    </div>
                                </div>
                            ))}
                    </div>
                            ...
                            <tbody className="stagger-rows">
                                ...
                                {!loading &&
                                    ...
                                    enquiries.map(e => (
                                        <tr
                                            key={e.id}
                                            className="border-b hover:bg-gray-50 align-top cursor-pointer"
                                            onClick={() => openViewModal(e)}
                                        >
                                            {/* Event Details */}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-700">
                            <div>
                                ...
                                <p className="font-medium">
                                    {selectedEnquiry.eventName ||
                                        (selectedEnquiry.title as string | undefined) ||
                                        "-"}
                                </p>
                            </div>
                            <div>
                                ...
                                <p className="font-medium">
                                    {clientMap.get(selectedEnquiry.client) ||
                                        selectedEnquiry.client}
                                </p>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <p className="font-medium">
                                    {selectedEnquiry.fromDate
                                        ? new Date(selectedEnquiry.fromDate).toLocaleString()
                                        : "TBD"}{" "}
                                    →{" "}
                                    {selectedEnquiry.toDate
                                        ? new Date(selectedEnquiry.toDate).toLocaleString()
                                        : "TBD"}
                                </p>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <p className="font-medium">
                                    {selectedEnquiry.poc || selectedEnquiry.enquiryPoC || "-"}
                                </p>
                            </div>
                            <div>
