...
export function ChecklistModal({ isOpen, onClose, eventData, onSave }: ChecklistModalProps) {
    ...
    useEffect(() => {
        ...
        if (eventData.checklist && eventData.checklist.length > 0) {
            ...
            const initialChecklist: ChecklistItem[] = eventData.items.map(item => ({
                ...
                vendor: item.vendor || '',
                status: 'PENDING',
                startDate: item.startDate || item.starDate ? String(item.startDate || item.starDate).split('T')[0] : '',
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const filteredData = checklistData.filter(item => item.item.trim() !== '' || item.description?.trim() !== '')
            ...
            <div className="bg-white dark:!bg-gray-900 border dark:border-gray-800 rounded-xl shadow-2xl w-full max-w-6xl max-h-[90vh] overflow-hidden flex flex-col relative m-4">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b shrink-0 bg-gray-50 dark:bg-gray-800">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">Manage execution tasks for {eventData.title}</p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="flex-1 overflow-y-auto p-6 bg-gray-50 dark:bg-gray-900">
                    {categories.length === 0 ? (
                        <div className="text-center py-12">
                            <p className="text-gray-500 mb-4">No items found in estimate. Add a category to start your checklist.</p>
                            <Button onClick={() => handleAddItem('New Category')}>Add First Item</Button>
                        </div>
                    ) : (
                        <Accordion type="multiple" className="space-y-4">
                            {categories.map((category, catIndex) => {
                                ...
                                return (
                                    <AccordionItem key={category} value={category} className="border border-gray-200 dark:border-gray-800 rounded-lg bg-white dark:bg-gray-900 overflow-hidden shadow-sm">
                                        <AccordionTrigger className="px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-800 font-semibold bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-200 data-[state=open]:border-b">
                                            <div className="flex items-center gap-3">
                                                ...
                                                {category}
                                                <span className="text-xs font-normal text-muted-foreground ml-2">({items.length} items)</span>
                                            </div>
                                        </AccordionTrigger>
                                            ...
                                            <div className="p-4 space-y-4">
                                                {items.map((item, idx) => (
                                                    <div
                                                        ...
                                                        className={`border rounded-xl p-4 transition-all ${item.status === 'DONE'
                                                    ...
                                                    >
                                                        ...
                                                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4 border-b dark:border-gray-800 pb-3">
                                                            <div className="flex-1 w-full grid grid-cols-1 sm:grid-cols-2 gap-3 items-center">
                                                                <div>
                                                                    <Label className="text-[11px] uppercase tracking-wider text-muted-foreground font-semibold mb-1 cursor-default">Item Name</Label>
                                                                    <Input
                                                                        value={item.item}
                                                                        onChange={e => handleItemChange(item.originalIndex, 'item', e.target.value)}
                                                                ...
                                                                <div>
                                                                    <Label className="text-[11px] uppercase tracking-wider text-muted-foreground font-semibold mb-1 cursor-default">Subcategory (Optional)</Label>
                                                                    <Input
                                                                        value={item.subCategory}
                                                                        onChange={e => handleItemChange(item.originalIndex, 'subCategory', e.target.value)}
                                                                ...
                                                                <div className="w-[140px]">
                                                                    <select
                                                                        ...
                                                                        className={`h-8 w-full rounded-md border text-[12px] font-semibold px-2 cursor-pointer focus:ring-2 focus:ring-ring outline-none transition-colors
                                                                            ${item.status === 'DONE' ? 'bg-green-100 text-green-800 border-green-200 dark:bg-green-900/30 dark:text-green-300 dark:border-green-800' :
                                                                                item.status === 'CONFIRMED' ? 'bg-blue-100 text-blue-800 border-blue-200 dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-800' :
                                                                                    item.status === 'IN_PROGRESS' ? 'bg-amber-100 text-amber-800 border-amber-200 dark:bg-amber-900/30 dark:text-amber-400 dark:border-amber-800' :
                                                        ...
                                                        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 bg-gray-50/80 dark:bg-gray-900/80 p-3 rounded-lg border dark:border-gray-800">
                                                            <div className="col-span-1">
                                                                <Label className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold mb-1 cursor-default">Quantity</Label>
                                                                <Input
                                                                    type="number"
                                                                    value={item.quantity}
                                                                    onChange={e => handleItemChange(item.originalIndex, 'quantity', parseInt(e.target.value) || 0)}
                                                                    className="h-8 text-[13px] bg-white dark:bg-gray-950"
                                                                />
                                                            </div>
                                                            <div className="col-span-1">
                                                                <Label className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold mb-1 cursor-default">Start Date</Label>
                                                                <Input
                                                                    type="date"
                                                                    value={item.startDate || ''}
                                                                    onChange={e => handleItemChange(item.originalIndex, 'startDate', e.target.value)}
                                                                    className="h-8 text-[12px] px-2 bg-white dark:bg-gray-950"
                                                                />
                                                            </div>
                                                            <div className="col-span-1">
                                                                <Label className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold mb-1 cursor-default">End Date</Label>
                                                                <Input
                                                                    type="date"
                                                                    value={item.endDate || ''}
                                                                    onChange={e => handleItemChange(item.originalIndex, 'endDate', e.target.value)}
                                                                    className="h-8 text-[12px] px-2 bg-white dark:bg-gray-950"
                                                                />
                                                            </div>
                                                            <div className="col-span-1">
                                                                <Label className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold mb-1 cursor-default">Time</Label>
                                                                <Input
                                                                    type="time"
                                                                    value={item.executionTime || ''}
                                                                    onChange={e => handleItemChange(item.originalIndex, 'executionTime', e.target.value)}
                                                                    className="h-8 text-[12px] px-2 bg-white dark:bg-gray-950"
                                                                />
                                                            </div>
                                                            <div className="col-span-2 lg:col-span-1 shrink-0">
                                                                <Label className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold mb-1 cursor-default">Assigned Vendor</Label>
                                                                <CreatableSelect
                                                                    options={vendorList.map(v => ({ value: String(v.id || ''), label: v.name as string }))}
                                                                    value={item.vendor
                                                                        ? { value: item.vendor, label: vendorList.find(v => String(v.id) === item.vendor)?.name || item.vendor }
                                                                        : null}
                                                                    onChange={(selected: any) => handleItemChange(item.originalIndex, 'vendor', selected ? selected.value : '')}
                ...
                <div className="px-6 py-4 border-t shrink-0 flex justify-between items-center bg-white dark:bg-gray-900 rounded-b-xl">
                    <div className="text-sm text-gray-500 font-medium">
                        Total Items: {checklistData.length} | Done: {checklistData.filter(i => i.status === 'DONE').length}
                    </div>
