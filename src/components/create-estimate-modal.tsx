...
import { CreateEstimatePayload, EstimateDto, EstimateItem, EstimateLineItemPayload, EstimateStatus } from "@/types/estimate"
...
const normaliseClientSummaryPayload = (payload: unknown): ClientEnquirySummary[] => {
    ...
    return list
        .map(entry => {
            ...
            const clientId = (() => {
                if (typeof item.clientId === "string" && item.clientId.trim()) return item.clientId.trim()
                return undefined
            })()

            const clientName = (() => {
                if (typeof item.clientName === "string" && item.clientName.trim()) return item.clientName.trim()
                if (typeof item.client === "string" && item.client.trim()) return item.client.trim()
                return undefined
            })()
            ...
            const enquiries = (() => {
                ...
                return raw
                    .map(enquiry => {
                        ...
                        const enquiryId = (() => {
                            if (typeof enq.enquiryId === "string" && enq.enquiryId.trim()) return enq.enquiryId.trim()
                            if (typeof enq.id === "string" && enq.id.trim()) return enq.id.trim()
                            return undefined
                        })()
                        const title = (() => {
                            if (typeof enq.title === "string" && enq.title.trim()) return enq.title.trim()
                            return undefined
                        })()
...
const extractFirstEnquiryRecord = (payload: unknown): Record<string, unknown> | undefined => {
    ...
    if (Array.isArray(payload)) {
        return payload.find(item => item && typeof item === "object" && !Array.isArray(item)) as Record<string, unknown> | undefined
    }
    if (typeof payload === "object") {
        const objectPayload = payload as Record<string, unknown>
        const candidateKeys = ["content", "items", "data", "enquiries", "results", "records", "list"]
        ...
        const hasUsefulField = ["id", "enquiryId", "title", "enquiryTitle"].some(key => key in objectPayload)
        if (hasUsefulField) {
...
const validationSchema = Yup.object({
    ...
    pocContactNumber: Yup.string().matches(/^\d{10}$/u, "Enter a 10 digit number").required("POC contact number is required"),
    enquiryPoC: Yup.string().optional(),
    status: Yup.mixed<EstimateStatus>().oneOf(statusOptions.map(s => s.value)).required(),
})
...
export default function CreateEstimateModal({ isOpen, onClose, initialData, onSaved }: CreateEstimateModalProps) {
    const [prefillData, setPrefillData] = useState<(Partial<EstimateDto> & { enquiryId?: string }) | undefined>(initialData)
    const [selectedClientId, setSelectedClientId] = useState(initialData?.client?.id ?? "")
    const [selectedEnquiryId, setSelectedEnquiryId] = useState<string | undefined>(initialData?.enquiryId)
    ...
    useEffect(() => {
        if (initialData?.client?.name && initialData.title) {
            ...
            enquiryPrefillCache.current.set(key, { prefill: initialData, id: initialData.enquiryId })
        }
    }, [initialData])
    ...
    useEffect(() => {
        ...
        const loadSummaries = async () => {
            ...
            try {
                ...
                if (!cancelled) {
                    setSummaryError(error instanceof Error ? error.message : "Unable to load enquiries summary")
                    setClientSummaries([])
                }
            } finally {
                if (!cancelled) {
    ...
    const mapEnquiryToPrefill = useCallback((record: Record<string, unknown> | null | undefined, fallback?: { id?: string; clientId?: string; clientName?: string; title?: string }) => {
        ...
        const cleanedItems = rawItems && typeof rawItems === "object"
            ? Object.entries(rawItems).reduce<Record<string, EstimateItem[]>>((acc, [category, entries]) => {
                ...
                acc[category] = entries.map((item, index) => {
                    ...
                    const description = typeof content.description === "string"
                        ...
                        : typeof content.item === "string"
                            ? content.item
                            : "Line item"
                    const specification = typeof content.specification === "string" ? content.specification : undefined
                    const rawDays = toNumber(content.days) ?? toNumber(content.duration) ?? 1
                    const rawSqft = toNumber(content.sqft) ?? toNumber(content.quantity) ?? 1
                    const rawRate = toNumber(content.rate) ?? toNumber(content.unitCost) ?? 0
                    ...
                    const safeDays = typeof rawDays === "number" && Number.isFinite(rawDays) && rawDays > 0 ? rawDays : 1
                    const safeSqft = typeof rawSqft === "number" && Number.isFinite(rawSqft) && rawSqft > 0 ? rawSqft : 1
                    const safeRate = typeof rawRate === "number" && Number.isFinite(rawRate) ? rawRate : 0
                    const safeQuantity = typeof rawQuantity === "number" && Number.isFinite(rawQuantity) && rawQuantity > 0 ? rawQuantity : safeSqft
                    const safeUnitCost = typeof rawUnitCost === "number" && Number.isFinite(rawUnitCost) ? rawUnitCost : safeRate
                    const safeTotal = typeof rawTotal === "number" && Number.isFinite(rawTotal)
        ...
        return {
            ...
            title: pick<string>(["title", "eventName", "summary", "name", "eventTitle"]) ?? fallback?.title,
            highlvelRequirement: pick<string>(["highlvelRequirement", "summary", "title", "description"]) ?? "",
            enquiryDate: pick<string>(["enquiryDate", "createdAt", "created_date", "createdDate"]),
            ...
            pocContactNumber: pick<string>(["enquiryPoCNumber", "pocContactNumber", "clientPhone", "clientPocPhone"]) ?? "",
    ...
    const loadEnquiryForSelection = useCallback(async (selection: { clientName: string; title: string }) => {
        ...
        try {
            ...
            const response = await apiRequest(`${API_ENDPOINTS.enquiries.list}?${params.toString()}`)
            if (!response.ok) {
                throw new Error(`Unable to load enquiry for ${clientName} • ${title}. (${response.status})`)
            }
            ...
            const nextPrefill = mapEnquiryToPrefill(record, { id: fallbackId, clientId: selectedClientId, clientName, title })
            if (!nextPrefill) {
                throw new Error("Enquiry response did not contain usable details.")
            }

            const currentSelection = selectionRef.current
            if (!currentSelection || currentSelection.clientName !== clientName || currentSelection.title !== title || requestRef.current !== requestId) {
                return
            }

            const resolvedId = nextPrefill.enquiryId ?? fallbackId
            if (!resolvedId) {
                throw new Error("The selected enquiry did not include an identifier. Please try another enquiry or refresh.")
            }
            ...
            const message = error instanceof Error ? error.message : "Failed to load enquiry details."
            console.error("Failed to prefill estimate from enquiry selection", error)
            const currentSelection = selectionRef.current
            if (currentSelection && currentSelection.clientName === clientName && currentSelection.title === title) {
    ...
    useEffect(() => {
        ...
        if (prefillData?.items) {
            Object.entries(prefillData.items).forEach(([category, items]) => {
                items.forEach((item, index) => {
                    const derivedId = typeof item.id === "string" || typeof item.id === "number"
                        ? String(item.id)
                        : `${category}-${index}-${generateId()}`
                    const description = typeof item.description === "string" ? item.description : "Line item"
                    const specification = typeof item.specification === "string" ? item.specification : ""
                    const rawQuantity = typeof item.quantity === "number" ? item.quantity : Number(item.quantity ?? 1)
                    const rawUnitCost = typeof item.unitCost === "number" ? item.unitCost : Number(item.unitCost ?? 0)
                    const rawDays = typeof item.days === "number" ? item.days : Number(item.days ?? 1)
                    const rawSqft = typeof item.sqft === "number" ? item.sqft : Number(item.sqft ?? rawQuantity ?? 1)
                    const rawRate = typeof item.rate === "number" ? item.rate : Number(item.rate ?? rawUnitCost ?? 0)
        ...
        if (fromItems.length === 0) {
            fromItems.push({
                id: generateId(),
                category: "General",
                item: prefillData?.highlvelRequirement ? prefillData.highlvelRequirement : "New line item",
                specification: "",
                days: 1,
                sqft: 1,
                rate: 0,
    ...
    const totalAmount = useMemo(() => lines.reduce((sum, line) => sum + (line.days * line.sqft * line.rate), 0), [lines])
    ...
    const enquiryStatusMessage = useMemo(() => {
        ...
        if (selectedClientId && titlesForSelectedClient.length === 0) return "No enquiries found for this client yet."
        if (isFetchingEnquiry) return "Loading enquiry details…"
        if (prefillData?.enquiryId || selectedEnquiryId) {
            return currentPrefillLabel ? `Prefilling from ${currentPrefillLabel}.` : "Prefilling from the selected enquiry."
        }
        return "Select an enquiry title to prefill the estimate fields."
    }, [currentPrefillLabel, clientSummaries.length, isFetchingEnquiry, prefillData?.enquiryId, selectedClientId, selectedEnquiryId, summaryError, summaryLoading, titlesForSelectedClient.length])

    const clearEnquirySelection = useCallback(() => {
        if (!prefillData && !selectedEnquiryId) {
            selectionRef.current = selectedClientId ? { clientName: clientSummaries.find(c => c.clientId === selectedClientId)?.clientName } : null
            return
        }
    ...
    const handleClientChange = useCallback((event: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        setPrefillData(undefined)
        setIsFetchingEnquiry(false)
        selectionRef.current = nextClientId ? { clientName: clientSummaries.find(c => c.clientId === nextClientId)?.clientName } : null
    }, [clientSummaries])

    const handleEnquiryTitleChange = useCallback((event: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        const selectedEnquiry = titlesForSelectedClient.find(e => e.enquiryId === nextEnquiryId)
        if (!selectedEnquiry) return

        setSelectedEnquiryId(nextEnquiryId)
        setPrefillData(undefined)
        selectionRef.current = { clientName: clientSummaries.find(c => c.clientId === selectedClientId)?.clientName, title: selectedEnquiry.title }
        void loadEnquiryForSelection({ clientName: clientSummaries.find(c => c.clientId === selectedClientId)?.clientName ?? "", title: selectedEnquiry.title })
    }, [clearEnquirySelection, loadEnquiryForSelection, selectedClientId, titlesForSelectedClient, clientSummaries])
        ...
        try {
            ...
            if (!activeEnquiryId || !prefillData) {
                ...
                toast.error("Enquiry required", {
                    description: "Choose an enquiry from the list so we can prefill the estimate details.",
                })
                setIsSaving(false)
                return
            }
            ...
            if (!resolvedClientId) {
                helpers.setStatus("The selected enquiry is missing a client id. Please refresh and try again.")
            ...
            const requestItems = lines.reduce<Record<string, EstimateLineItemPayload[]>>((acc, line) => {
                ...
                const count = Number.isFinite(line.sqft) && line.sqft > 0 ? Number(line.sqft) : 1
                    ...
                    {({ values, setFieldValue, status }) => (
                        <Form>
                            ...
                            <div className="px-6 py-4 space-y-6">
                                {status && <div className="rounded-md border border-red-200 bg-red-50 text-red-700 px-4 py-3 text-sm">{status}</div>}

                                <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                                    <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                                        <div className="w-full md:max-w-md">
                                            <Label className="text-xs uppercase tracking-wide text-muted-foreground">Source enquiry</Label>
                                            <div className="mt-1 flex flex-col gap-2 lg:flex-row lg:items-center">
                                                ...
                                                >
                                                    <option value="">{summaryLoading ? "Loading clients…" : "Pick a client…"}</option>
                                                ...
                                                <div className="flex w-full gap-2 lg:w-auto lg:flex-1">
                                                    <select
                                                        ...
                                                        disabled={!selectedClientId || summaryLoading || isFetchingEnquiry || titlesForSelectedClient.length === 0}
                                                        className="flex-1 rounded-md border border-gray-300 bg-white px-3 py-2 text-sm shadow-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500"
                                                    >
                                                        <option value="">
                                                            {!selectedClientId
                                            ...
                                            <p className="mt-2 text-xs text-muted-foreground">{enquiryStatusMessage}</p>
                                        </div>
                                        <div className="text-left md:text-right">
                                            <span className="text-xs uppercase tracking-wide text-muted-foreground">Estimate total</span>
                                            <p className="text-xl font-semibold text-blue-600">₹{totalAmount.toFixed(2)}</p>
                                            {prefillData?.client?.name && (
                                                <p className="mt-1 text-xs text-muted-foreground">Client • {prefillData.client.name}</p>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    <div className="mt-4 space-y-2">
                                        <Label htmlFor="highlvelRequirement" className="text-sm font-medium text-gray-700">Project summary</Label>
                                        <Field
                                            as="textarea"
                                        ...
                                        />
                                        <ErrorMessage name="highlvelRequirement" component="div" className="text-sm text-red-600" />
                                    </div>
                                </section>

                                <section className="grid gap-4 md:grid-cols-2">
                                    <div className="rounded-xl border border-gray-200 bg-slate-50 p-4">
                                        <h4 className="text-sm font-semibold text-gray-700">Event schedule</h4>
                                        <div className="mt-3 grid gap-3">
                                            <div>
                                                <Label className="text-xs uppercase tracking-wide text-muted-foreground">Enquiry Date</Label>
                                                <DatePicker
                                                    selected={values.enquiryDate ?? null}
                                                    onChange={(date: Date | null) => setFieldValue("enquiryDate", date)}
                                                    showTimeSelect
                                                    timeIntervals={15}
                                            ...
                                            <div className="grid gap-3 md:grid-cols-2">
                                                <div>
                                                    <Label className="text-xs uppercase tracking-wide text-muted-foreground">Event Start</Label>
                                                    <DatePicker
                                                ...
                                                <div>
                                                    <Label className="text-xs uppercase tracking-wide text-muted-foreground">Event End</Label>
                                                    <DatePicker
                                    ...
                                    <div className="rounded-xl border border-gray-200 bg-white p-4">
                                        <h4 className="text-sm font-semibold text-gray-700">Logistics</h4>
                                        <div className="mt-3 grid gap-3">
                                            <div>
                                                <Label htmlFor="status" className="text-xs uppercase tracking-wide text-muted-foreground">Status</Label>
                                                <Field as="select" id="status" name="status" className="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm">
                                            ...
                                            <div className="grid gap-3 md:grid-cols-2">
                                                <div>
                                                    <Label htmlFor="venue" className="text-xs uppercase tracking-wide text-muted-foreground">Venue</Label>
                                                ...
                                                <div>
                                                    <Label htmlFor="location" className="text-xs uppercase tracking-wide text-muted-foreground">Location</Label>
                                                    <Field as={Input} id="location" name="location" placeholder="Enter event location" className="mt-1" />
                                ...
                                <section className="rounded-xl border border-gray-200 bg-white p-4">
                                    ...
                                    <div className="mt-3 grid gap-3 md:grid-cols-3">
                                        <div>
                                            <Label htmlFor="clientPoC" className="text-xs uppercase tracking-wide text-muted-foreground">Client POC</Label>
                                            <Field as={Input} id="clientPoC" name="clientPoC" placeholder="Client point of contact" className="mt-1" />
                                            <ErrorMessage name="clientPoC" component="div" className="mt-1 text-sm text-red-600" />
                                        </div>
                                        <div>
                                            <Label htmlFor="pocContactNumber" className="text-xs uppercase tracking-wide text-muted-foreground">POC Contact Number</Label>
                                            <Field as={Input} id="pocContactNumber" name="pocContactNumber" placeholder="1234567890" maxLength={10} className="mt-1" />
                                            <ErrorMessage name="pocContactNumber" component="div" className="mt-1 text-sm text-red-600" />
                                        </div>
                                        <div>
                                            <Label htmlFor="enquiryPoC" className="text-xs uppercase tracking-wide text-muted-foreground">Enquiry POC</Label>
                                            <Field as={Input} id="enquiryPoC" name="enquiryPoC" placeholder="Internal assignee" className="mt-1" />
                                        </div>
                                    </div>
                                </section>
                                ...
                                <section className="space-y-4">
                                    <div className="flex flex-col gap-2 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
                                        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                                            <div>
                                                <h3 className="text-base font-semibold text-gray-900">Artifacts required</h3>
                                                <p className="text-xs text-muted-foreground">List the services, equipment, and resources needed for this estimate.</p>
                                            </div>
                                            <Button
                                                type="button"
                                                variant="outline"
                                                ...
                                                onClick={() => setLines(prev => {
                                                    const lastCategory = prev.length > 0 ? prev[prev.length - 1].category : "General"
                                                    return [
                                                        ...prev,
                                                        {
                                                            id: generateId(),
                                                ...
                                                <tbody>
                                                    {lines.map((line, index) => (
                                                        <tr key={line.id} className="border-b last:border-0">
                                                            <td className="py-3 pr-3 align-middle text-xs text-muted-foreground">{index + 1}</td>
                                                            <td className="py-3 pr-3 align-middle">
                                                                <Input value={line.category} onChange={(event) => {
                                                                    const value = event.target.value
                                                                    setLines(prev => prev.map((l, idx) => idx === index ? { ...l, category: value } : l))
                                                                }} />
                                                            </td>
                                                            <td className="py-3 pr-3 align-middle">
                                                                <Input value={line.item} onChange={(event) => {
                                                                    const value = event.target.value
                                                                    setLines(prev => prev.map((l, idx) => idx === index ? { ...l, item: value } : l))
                                                                }} />
                                                            </td>
                                                            <td className="py-3 pr-3 align-middle">
                                                                <Input value={line.specification} onChange={(event) => {
                                                                    const value = event.target.value
                                                            ...
                                                            <td className="py-3 pr-3 align-middle">
                                                                <Input
                                                                    ...
                                                                    onChange={(event) => {
                                                                        ...
                                                                        setLines(prev => prev.map((l, idx) => idx === index ? { ...l, days: Number.isNaN(value) ? 0 : value } : l))
                                                                    }}
                                                                />
                                                            </td>
                                                            <td className="py-3 pr-3 align-middle">
                                                                <Input
                                                                    ...
                                                                    onChange={(event) => {
                                                                        const value = Number(event.target.value)
                                                                        setLines(prev => prev.map((l, idx) => idx === index ? { ...l, sqft: Number.isNaN(value) ? 0 : value } : l))
                                                                    }}
                                                                />
                                                            </td>
                                                            <td className="py-3 pr-3 align-middle">
                                                                <Input
                                                                    ...
                                                                    onChange={(event) => {
                                                                        const value = Number(event.target.value)
                                                                        setLines(prev => prev.map((l, idx) => idx === index ? { ...l, rate: Number.isNaN(value) ? 0 : value } : l))
                                                                    }}
                                                                />
                                                            </td>
                                                            <td className="py-3 pr-3 align-middle text-right font-semibold">
                                                                ₹{(line.days * line.sqft * line.rate).toFixed(2)}
                                                            </td>
                                                            <td className="py-3 text-right align-middle">
                                                                <Button type="button" variant="ghost" size="sm" disabled={lines.length === 1} onClick={() => setLines(prev => prev.filter((_, idx) => idx !== index))}>
                                                                    Remove
                                                                </Button>
                                                            </td>
                                                        </tr>
                                    ...
                                    <div className="flex justify-end rounded-xl bg-slate-900/90 px-5 py-4 text-sm text-slate-100">
                                        <div className="flex items-center gap-3">
                                            <span className="uppercase tracking-wide text-xs text-slate-300">Cost Summary</span>
