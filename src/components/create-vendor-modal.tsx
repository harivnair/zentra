...
export default function CreateVendorModal({ isOpen, onClose, onSubmit, editData, mode = 'create' }: CreateVendorModalProps) {
    ...
    const handleSubmit = async (values: VendorFormData, { setSubmitting, setStatus }: FormikHelpers<VendorFormData>) => {
        try {
            setStatus(null)
            ...
            if (!res.ok) {
                ...
                throw new Error(text || `Failed to ${isEdit ? 'update' : 'create'} vendor: ${res.status}`)
            }
                        ...
                        <Form>
                            ...
                            <div className="flex items-center justify-between mb-4">
                                <div className="flex items-center gap-3">
                                    <div className="h-10 w-10 rounded bg-orange-100 flex items-center justify-center">🚚</div>
                                    <div>
                                        <h2 className="text-xl font-semibold">{isEdit ? 'Edit Vendor' : 'Add New Vendor'}</h2>
                                        <p className="text-sm text-gray-600">{isEdit ? 'Update vendor details' : 'Fill in the vendor details below'}</p>
                                    </div>
                                </div>
                            ...
                            <div className="pt-4 space-y-4">
                                ...
                                {/* Vendor Name */}
                                <div>
                                    <Label htmlFor="name">Vendor Name <span className="text-red-600">*</span></Label>
                                    <Field
                                        as={Input}
                                        id="name"
                                        name="name"
                                ...
                                <div>
                                    ...
                                    <Field name="phone">
                                        {({ field, form }: { field: { name: string; value: string }; form: { setFieldValue: (name: string, value: string) => void } }) => (
                                            <Input
                                                {...field}
                                                id="phone"
                                                type="tel"
                                ...
                                {/* Billing Address */}
                                <div>
                                    <Label htmlFor="billingAddress">Billing Address <span className="text-red-600">*</span></Label>
                                    <Field
                                        as="textarea"
                                        id="billingAddress"
                                        name="billingAddress"
                                    ...
                                    />
                                    <ErrorMessage name="billingAddress" component="div" className="mt-1 text-sm text-red-600" />
                                </div>

                                {/* GST and TDS */}
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <Label htmlFor="gst">GST (%) <span className="text-red-600">*</span></Label>
                                        <Field
                                            as={Input}
                                            id="gst"
                                            name="gst"
                                            ...
                                            className="mt-1"
                                        />
                                        <ErrorMessage name="gst" component="div" className="mt-1 text-sm text-red-600" />
                                    </div>
                                    <div>
                                        <Label htmlFor="tds">TDS (%) <span className="text-red-600">*</span></Label>
                                        <Field
                                            as={Input}
                                            id="tds"
                                            name="tds"
                                            ...
                                            className="mt-1"
                                        />
                                        <ErrorMessage name="tds" component="div" className="mt-1 text-sm text-red-600" />
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <Field
                                        ...
                                        className="mt-1"
                                    />
                                    <ErrorMessage name="gstCertificate" component="div" className="mt-1 text-sm text-red-600" />
                                    <p className="mt-1 text-xs text-gray-500">Optional - Enter the GST registration number if available</p>
                                </div>
                            </div>
