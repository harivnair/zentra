...
import {
    Home, FileText, Calendar, Grid, DollarSign,
    BookOpen, CreditCard, BarChart2, Users, Truck,
...
export default function Sidebar() {
    ...
    const NavLink = ({ item }: { item: NavItem }) => {
        ...
        return (
            ...
            >
                ...
                {item.Icon && (
                    <item.Icon className={`w-4 h-4 shrink-0 transition-opacity ${active ? 'opacity-100' : 'opacity-70 group-hover:opacity-100'}`} />
                )}

                <span className="flex-1 truncate">{item.label}</span>

                {loading && (
                    <svg className="animate-spin h-3.5 w-3.5 text-slate-400 shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
    ...
    return (
        <aside className="hidden md:flex w-60 flex-col acrylic-light text-slate-900 py-5 px-3 min-h-screen shrink-0">
            {/* Logo */}
            ...
            <div className="flex-1 overflow-y-auto hide-scrollbar flex flex-col gap-0.5">
                {topItems.map(it => <NavLink key={it.href} item={it} />)}

                <SectionLabel label="Sales" />
                {salesAndMarketing.map(it => <NavLink key={it.href} item={it} />)}

                <SectionLabel label="Operations" />
                {operations.map(it => <NavLink key={it.href} item={it} />)}

                <SectionLabel label="Finance" />
                {finance.map(it => <NavLink key={it.href} item={it} />)}
            </div>
            ...
            <div className="mt-4 border-t border-black/5 pt-3">
                    ...
                    <div className="px-3 py-2 mb-1">
                        <div className="flex items-center gap-2">
                            ...
                            <div className="min-w-0">
                                <p className="text-xs font-medium text-slate-900 truncate">{user.name || user.uid}</p>
                ...
                <button
                    ...
                    className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-slate-600 hover:text-red-600 hover:bg-red-50 transition-all duration-150"
                >
