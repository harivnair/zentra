...
export function EnquiriesProvider({ children }: { children: React.ReactNode }) {
    ...
    async function fetchEnquiries() {
        // Reuse the same promise if already fetching to deduplicate requests
        if (fetchPromiseRef.current !== null) {
        ...
        fetchPromiseRef.current = (async () => {
            try {
                ...
                // Query for ENQUIRY_CREATED status
                const res = await apiRequest(
                    `${API_ENDPOINTS.enquiries.list}?page=0&size=10&status=ENQUIRY_CREATED`
                );
                ...
                const list = (rawList as unknown[]).map(item => {
                    ...
                    const idVal =
                        get(["id", "_id", "enquiryId", "id_enquiry"]) ?? JSON.stringify(it);
                    ...
                    if (it.client && typeof it.client === "object") {
                        ...
                        if (c && "name" in c && typeof c.name === "string")
                            clientNameFromObj = c.name;
                    }
